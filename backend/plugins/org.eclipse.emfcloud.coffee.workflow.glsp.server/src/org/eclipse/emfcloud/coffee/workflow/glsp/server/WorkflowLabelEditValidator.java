...
public class WorkflowLabelEditValidator implements LabelEditValidator {
	...
	public ValidationStatus validate(final GModelState modelState, final String label, final GModelElement element) {
		...
		if (featureId != null) {
			...
			if(constraints != null) {
				return checkConstraints(constraints, label, element);
			}
		}
	...
	private String getElementId(String type) {
		if (type.equals(ModelTypes.AUTOMATED_TASK)) {
			return "http://www.eclipse.org/emfcloud/coffee/model#//AutomaticTask";
		}
		if (type.equals(ModelTypes.MANUAL_TASK)) {
			return "http://www.eclipse.org/emfcloud/coffee/model#//ManualTask";
		}
		return null;
	}
